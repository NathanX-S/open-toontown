...
class DistributedBuilding(DistributedObject.DistributedObject):
    ...
    def enterWaitForVictors(self, ts):
        ...
        if victorCount == 1:
            ...
            base.setFov(ToontownGlobals.DefaultCameraFov)
            anyOthers = 0
    ...
    def walkOutCameraTrack(self):
        track = Sequence(Func(camera.reparentTo, render), Func(camera.setPosHpr, self.elevatorNodePath, 0, -32.5, 9.4, 0, 348, 0), Func(base.setFov, ToontownGlobals.DefaultCameraFov), Wait(VICTORY_RUN_TIME), Func(camera.setPosHpr, self.elevatorNodePath, 0, -32.5, 17, 0, 347, 0), Func(base.setFov, 75.0), Wait(TO_TOON_BLDG_TIME), Func(base.setFov, ToontownGlobals.DefaultCameraFov))
        return track
